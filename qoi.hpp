...
constexpr auto QOI_COLOR_HASH(const auto& C) {
...
constexpr unsigned int QOI_MAGIC = []() -> unsigned int {
			...
			if (px.v == px_prev.v) {
				...
				if (index[index_pos].v == px.v) {
					...
					if (px.rgba.a == px_prev.rgba.a) {
						const signed char vr = px.rgba.r - px_prev.rgba.r;
						const signed char vg = px.rgba.g - px_prev.rgba.g;
						const signed char vb = px.rgba.b - px_prev.rgba.b;
	...
	constexpr /*std::vector<uint8_t>*/ void* qoi_decode(const void *data, const size_t size, qoi_desc *desc, int channels) {
		...
		auto *pixels = static_cast<unsigned char *>(QOI_MALLOC(px_len));
		if (!pixels) {
			return nullptr;
		}
